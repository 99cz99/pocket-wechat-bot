...
adb shell "cd /sdcard/Download && tar czf - pocket-wechat-bot/ 2>/dev/null | run-as com.termux sh -c 'cd ~ && rm -rf pocket-wechat-bot && tar xzf -'" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "╔══════════════════════════════════════════════╗"
...
Write-Host "----------------------------------------"
Write-Host ""
adb shell "run-as com.termux sh -c 'cd ~/pocket-wechat-bot && chmod +x scripts/setup-phone.sh && DEPLOY_NONINTERACTIVE=1 ./scripts/setup-phone.sh'"
Write-Host ""
Write-Host "----------------------------------------"
Write-Host "  手机端输出结束"
Write-Host "----------------------------------------"
